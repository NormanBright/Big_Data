...
transaction_filter = transaction.filter(transaction)
transaction_join = transaction_filter.map(lambda field_num: (field_num.split(",")[2], int(field_num.split(",")[3])))
feature = transaction_join.reduceByKey(lambda a, b: a + b) #aggregation
...
join = feature.join(cont_join) #this actually join
